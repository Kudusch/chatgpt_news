...
When looking at the top domains, some plausible and expected choices are at the top (e.g., `reuters.com`, `tagesschau.de`), but also some surprises appear (e.g., `deutsche-handwerks-zeitung.de`). @tbl-correl displays Spearman's rank correlation comparing the ranking of ChatGPT's domain frequency and the Tranco ranks for these domains when only focusing at German top-level domains.^[To increase comparability with the Tranco ranks that have a global scope.]
...
cut_size <- length(colnames(reuters))
...
truncated |>
  group_by(condition) |>
  summarise(
    intersection = length(intersect(domain_reuters, colnames(reuters))),
    union = length(union(domain_reuters, colnames(reuters))),
...
How many of the Reuters sources appear in the top `r length(colnames(reuters))` entries in the ChatGPT list. Cutoff can be flexible, here it is determined by the size of the short Reuters list.

```{r}
truncated |>
  group_by(condition) |>
  summarise(
    overlap = length(intersect(domain_reuters, colnames(reuters))),
...
matched_domains <- purrr::map_dfr(
  colnames(reuters),
  ~ {
    leaning_value <- reuters["media_leaning", .x]  
    reuters_rank <- reuters["rank", .x] 
    domains |>
      filter(str_detect(domain_reuters, regex(paste0("\\b", .x, "\\b"), ignore_case = TRUE))) |>
      mutate(
...
Political orientation was assessed on a scale from 1 (= left) to 7 (right), *N* = 2047. See @Behre2025 for details on the 2025 German Reuters Digital News Report. The mean political orientation of participants was 4.03 (SD = 1.19), the mean political leaning of the 22 media outlets was `r round(rowMeans(reuters)[[1]],2)` (SD = `r round(sd(as.numeric(reuters[1, ])), 2)`).
