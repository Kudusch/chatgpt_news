...
When looking at the top domains, some plausible and expected choices are at the top (e.g., `reuters.com`, `tagesschau.de`), but also some surprises appear (e.g., `deutsche-handwerks-zeitung.de`). @tbl-correl displays Spearman's rank correlation comparing the ranking of ChatGPT's domain frequency and the Tranco ranks for these domains when only focusing at German top-level domains.^[This was made to increase comparability with the Tranco ranks that have a global scope.]

```{r}
#| echo: false
#| warning: false
#| label: tbl-correl
#| tbl-cap: Spearman's Rank Correlation between Domain Rank and Tranco Rank
#| 
...
domains |>
  ...
  group_by(condition) |>
  mutate(rank = min_rank(desc(n))) |>  # ranks n descending, ties get same rank
  summarise(
    spearman_tranco = round(cor(rank, tranco_rank, method = "spearman", use = "complete.obs"),3)
  ) |> 
  gt() |>
  cols_label(
        condition = "Condition",
        spearman_tranco = "Spearman's rank correlation")
...
media_leanings <- read.csv("data/media_leanings_reuters.csv",row.names = 1)

# group sat1/prosieben and ard/tagesschau as they are considered one in the Reuters data
# add frequency for those cases and assign rank
domains_reuters <- domains |> 
  mutate(domain_reuters = case_when(
    domain %in% c("ard.de", "tagesschau.de") ~ "ard.de.tagesschau.de",
    domain %in% c("sat1.de", "prosieben.de") ~ "sat1.de.prosieben.de",
    TRUE ~ domain
  )) |>
  group_by(condition, domain_reuters) |>
  summarise(n = sum(n, na.rm = TRUE),
            category = first(category),
            .groups = "drop") |>
  group_by(condition) |>
  mutate(rank_global = min_rank(desc(n))) |>
  ungroup()
```

#### Comparing Ranks - Option 1: Subset longer list to match entries of partial list

Here the basis is the partial list.
```{r}
# match domains from chatgpt with reuters data
matched_domains <- purrr::map_dfr(
  colnames(media_leanings),
  ~ {
    reuters_rank <- media_leanings["rank", .x] 
    domains_reuters |>
      filter(str_detect(domain_reuters, regex(paste0("\\b", .x, "\\b"), ignore_case = TRUE))) |>
      mutate(
        source_match = .x,
        reuters_rank = reuters_rank
      )
  }
)

# calculate chatgpt rank for this subset
matched_domains |>
  group_by(condition) |>
  mutate(rank_subset = min_rank(desc(n))) |>
  summarise(
    spearman_global = round(cor(rank_global, reuters_rank, method = "spearman", use = "complete.obs"), 3),
    spearman_subset = round(cor(rank_subset, reuters_rank, method = "spearman"),3),
    .groups = "drop"
  )

# irrelevant whether using global ranking or new ranking for subset as order remains stable
# similarly: filtering for journalistic sources in ChatGPT list not necessary as relative ranks won't change
```
...
cut_size <- length(colnames(media_leanings))

truncated <- domains_reuters |> 
...
truncated |>
  left_join(
    matched_domains |>
      select(domain_reuters, condition, reuters_rank) |>
      distinct(domain_reuters, condition, .keep_all = TRUE),
    by = c("domain_reuters", "condition")
  ) |>
  summarise(
    spearman_global = cor(rank_global, reuters_rank, method = "spearman", use = "complete.obs") |> round(3)
  )

```

#### Comparing Ranks - Option 3: Jaccard similarity

For a fixed cutoff (here: size of the Reuters list), compute Jaccard similarity (= size of intersection / size of union).

```{r}
truncated |>
  group_by(condition) |>
  summarise(
    intersection = length(intersect(domain_reuters, colnames(media_leanings))),
    union = length(union(domain_reuters, colnames(media_leanings))),
    jaccard = round(intersection / union, 3)
  )
```


#### Comparing Ranks - Option 4: Overlap at n

How many of the Reuters sources appear in the top `r length(colnames(media_leanings))` entries in the ChatGPT list. Cutoff can be flexible, here it is determined by the size of the short Reuters list.

```{r}
truncated |>
  group_by(condition) |>
  summarise(
    overlap = length(intersect(domain_reuters, colnames(media_leanings))),
    relative_overlap = round(overlap/cut_size,2)
  )
```
...
pairwise_agreement <- domains_reuters |>
  left_join(
    matched_domains |>
      select(domain_reuters, condition, reuters_rank) |>
      distinct(domain_reuters, condition, .keep_all = TRUE),
...
pairwise_agreement_domains <- function(domains) {
    items <- pairwise_agreement$domain_reuters
    rank1 <- pairwise_agreement$rank_global
    rank2 <- pairwise_agreement$reuters_rank
...
result <- pairwise_agreement_domains(pairwise_agreement)
...
## group sat1/prosieben and ard/tagesschau as they are considered one in the Reuters data
# adjust some sub domains for main domain
domains <- domains |> 
  mutate(domain_reuters = case_when(
    domain %in% c("ard.de", "tagesschau.de") ~ "ard.de.tagesschau.de",
    domain %in% c("sat1.de", "prosieben.de") ~ "sat1.de.prosieben.de",
...
matched_domains <- purrr::map_dfr(
  colnames(media_leanings),
  ~ {
    leaning_value <- media_leanings["media_leaning", .x]  
    reuters_rank <- media_leanings["rank", .x] 
    domains |>
      filter(str_detect(domain_reuters, regex(paste0("\\b", .x, "\\b"), ignore_case = TRUE))) |>
      mutate(
        source_match = .x,
        leaning = leaning_value,
        reuters_rank = reuters_rank
      )
  }
)

#delete wrong/irrelevant match
matched_domains <- subset(matched_domains, matched_domains$domain_reuters != "unsere-zeit.de")

# get share of media for which we have the leaning of total data 
share_leaning_all <- domains |>
  group_by(condition) |>
  summarise(total_n = sum(n, na.rm = TRUE), .groups = "drop") |>
  left_join(
    matched_domains |>
      group_by(condition) |>
      summarise(matched_n = sum(n, na.rm = TRUE), .groups = "drop"),
    by = "condition"
  ) |> 
  mutate(share = matched_n / total_n)

# the same but only considering journalistic media as the reference
share_leaning_journalistic <- domains |>
...
Political orientation was assessed on a scale from 1 (= left) to 7 (right), *N* = 2047. See @Behre2025 for details on the 2025 German Reuters Digital News Report. The mean political orientation of participants was 4.03 (SD = 1.19), the mean political leaning of the 22 media outlets was `r round(rowMeans(media_leanings)[[1]],2)` (SD = `r round(sd(as.numeric(media_leanings[1, ])), 2)`).
...
matched_domains |>
  group_by(condition) |>
  summarise(
    `Mean Outlet Leaning` = weighted.mean(leaning, n),
    `SD Media Leaning` = sqrt(Hmisc::wtd.var(leaning, n)),
