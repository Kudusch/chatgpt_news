...
When looking at the top domains, some plausible and expected choices are at the top (e.g., `reuters.com`, `tagesschau.de`), but some surprises appear, too (e.g., `deutsche-handwerks-zeitung.de`). 
...
```
**From here until 1.2 (the rank correlation things) are still experimental**
...
cut_size_similar <- nrow(similar)

similar_corr <- domains_similar |> 
  filter(category %in% c("Journalistic Media", "News Agencies", NA)) |> 
  group_by(condition) |> 
  arrange(rank) |>
  slice_head(n = cut_size_similar) |>
  summarise(
...
jacc_reu <- domains_reuters |> 
  ...
  summarise(
    ...
    overlap.r = round(intersection.r / cut_size, 2)
  )
...
jacc_sim <- domains_similar |> 
  ...
  summarise(
    ...
    overlap.s = round(intersection.s / cut_size_similar, 2)
  )
